...
import com.example.demo.works.HistoryWork;
...
public record SavedAdvertisementResponse(String workflowId, String generationId, GenerationStatus status,
    ...
    public static SavedAdvertisementResponse from(HistoryWork work) {
        var result = work.getContent().generation();
        var images = work.getContent().images();
        String url = "/dashboard/works/" + work.getId();
        return new SavedAdvertisementResponse(result.workflowId(), result.generationId(), result.status(),
                ...
                result.review(), result.decision(), work.getId(), url, IntStream.range(0, images.size()).mapToObj(i -> {
